...
Use [Phantom Ink `1398bd4`](https://github.com/smirea/phantom-ink/tree/1398bd43841fd9e7533f0106041169cb4cab9e8b) as the canonical implementation, including for Svelte. Inspect the current default branch for later improvements, then read the linked files and their imports before copying a pattern.
...
Let the server own and distribute authoritative room and game state. Let clients render the latest snapshot and send semantic actions such as `join`, `vote`, `pickWord`, or `guessLetter`. Never make a UI component the authority for whether an action is legal.

Keep all framework-free, deterministic logic in `packages/shared`:
...
## Build the complete game in the debug route first

Implement the full game locally in `/debug/game` before adding accounts, rooms, persistence, or networking. Still design for networking from the start: use serializable state and events, deterministic transitions, explicit viewer identity, and a single `send(event)` boundary.
...
## Copy identity storage exactly

Copy these files together, changing only the application namespace and storage shape:
...
## Add a lightweight player profile

Represent an account as a server record with `id`, `name`, `color`, and `icon`. Treat it as player presentation, not authentication.
...
## Add rooms and networking

Use one simple state flow:
...
### Make rejoining intentional

On startup, load `userId`, ask the server for `users.currentRoom`, and navigate to `/room/[code]` while preserving `DEBUG_ID`. Let the room route idempotently join again and reopen the event stream. A direct refresh follows the same path. If no room remains, continue to the lobby; send `leave` only for an explicit abandon action.
...
## Keep voting declarative

Keep lobby consensus separate from in-game decisions, even when both use the same presentation components.
...
Never accept a client-supplied vote count or consensus result.

## Expose live debug helpers

Expose a small `window.DEBUG` API while the room route is mounted:
